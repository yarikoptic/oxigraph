...
///
/// Support for [N-Quads-star](https://w3c.github.io/rdf-star/cg-spec/#n-quads-star) is available behind the `rdf-star` feature and the [`NQuadsParser::with_quoted_triples`] option.
///
...
/// let mut count = 0;
/// for triple in NQuadsParser::new().parse_from_read(file.as_ref()) {
///     let triple = triple?;
...
impl NQuadsParser {
    ...
    /// Enables [N-Triples-star](https://w3c.github.io/rdf-star/cg-spec/#n-quads-star).
    #[cfg(feature = "rdf-star")]
    ...
    /// Parses a N-Quads file from a [`Read`](std::io::Read) implementation.
    ///
    /// Count the number of people:
    /// ```
    /// use oxrdf::NamedNodeRef;
...
/// Parses a N-Quads file from a [`Read`](std::io::Read) implementation. Can be build using [`NQuadsParser::parse_from_read`].
///
/// Count the number of people:
/// ```
/// use oxrdf::NamedNodeRef;
...
/// Parses a N-Quads file by using a low-level API. Can be build using [`NQuadsParser::parse`].
///
/// Count the number of people:
/// ```
/// use oxrdf::NamedNodeRef;
