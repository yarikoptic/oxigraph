...
impl RuleRecognizer for NQuadsRecognizer {
        ...
        if let Some(state) = self.stack.pop() {
            match state {
                NQuadsState::ExpectSubject => match token {
                    ...
                    _ => self.error(
                        errors,
                        "The subject of a triple should be an IRI or a blank node",
                    ),
                },
                NQuadsState::ExpectPredicate => match token {
                    ...
                    _ => self.error(errors, "The predicate of a triple should be an IRI"),
                },
                NQuadsState::ExpectedObject => match token {
                    ...
                    _ => self.error(
                        errors,
                        "The object of a triple should be an IRI, a blank node or a literal",
                    ),
                },
    ...
    fn recognize_end(&self, _results: &mut Vec<Quad>, errors: &mut Vec<RuleRecognizerError>) {
        match self.stack.last() {
            Some(NQuadsState::ExpectSubject) | None => (),
            Some(NQuadsState::ExpectDot) => {
                errors.push("Triples should be followed by a dot".into())
            }
            Some(_) => errors.push("Unexpected end".into()), //TODO
        }
    }
