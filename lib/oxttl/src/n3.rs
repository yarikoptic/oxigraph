...
use oxrdf::Triple;
use oxrdf::{
    BlankNode, GraphName, Literal, NamedNode, NamedNodeRef, NamedOrBlankNode, Subject, Term,
    Variable,
};
...
impl N3Parser {
    pub fn builder() -> N3ParserBuilder {
        N3ParserBuilder {
            base: None,
    ...
    pub fn read_next(&mut self) -> Option<Result<N3Event, ParseError>> {
        self.parser.read_next()
    }
}

pub struct N3ParserBuilder {
    base: Option<Iri<String>>,
    prefixes: HashMap<String, Iri<String>>,
}

impl N3ParserBuilder {
    #[inline]
    ...
    pub fn with_read<R: Read>(self, read: R) -> N3FromRead<R> {
        N3FromRead {
            inner: self.build().parser.parse_from_read(read),
        }
    }
}

pub struct N3FromRead<R: Read> {
    inner: FromReadIterator<R, N3Recognizer>,
}

impl<R: Read> Iterator for N3FromRead<R> {
    type Item = Result<N3Event, ParseOrIoError>;

    fn next(&mut self) -> Option<Result<N3Event, ParseOrIoError>> {
        self.inner.next()
    }
}
        ...
        if let Some(rule) = self.stack.pop() {
            match rule {
                ...
                N3State::BaseExpectIri => match token {
                    N3Token::IriRef(iri) => {
                        self.lexer_options.base_iri = Some(iri.clone());
                        results.push(N3Event::Base(Base { iri: NamedNode::new_unchecked(iri.into_inner()) }));
                        self
                    }
                ...
                N3State::PrefixExpectPrefix => match token {
                    N3Token::PrefixedName(prefix, local) if local.is_empty() => {
                        self.stack.push(N3State::PrefixExpectIri { name: prefix.to_string() });
                        self
                    }
                    _ => {
                ...
                N3State::PrefixExpectIri { name } => match token {
                    N3Token::IriRef(iri) => {
                        self.prefixes.insert(name.clone(), iri.clone());
                        results.push(N3Event::Prefix(Prefix { name, iri: NamedNode::new_unchecked(iri.into_inner()) }));
                        self
                    }
                ...
                N3State::TriplesMiddle => if matches!(token, N3Token::Punctuation(".") | N3Token::Punctuation("]") | N3Token::Punctuation("}")| N3Token::Punctuation(")")) {
                    self.recognize_next(token, results, errors)
                } else {
                ...
                N3State::PredicateObjectListPossibleContinuation => if token == N3Token::Punctuation(";") {
                    self.stack.push(N3State::PredicateObjectListPossibleContinuation);
                    self
                } else if matches!(token, N3Token::Punctuation(";") | N3Token::Punctuation(".") | N3Token::Punctuation("}")| N3Token::Punctuation("]")| N3Token::Punctuation(")")) {
                    self.recognize_next(token, results, errors)
                } else {
                    self.stack.push(N3State::PredicateObjectListEnd);
                    self.stack.push(N3State::ObjectsList);
                    ...
                    match token {
                        ...
                        N3Token::PrefixedName(prefix, local) => {
                            if let Some(start) = self.prefixes.get(prefix) {
                                self.terms.push(NamedNode::new_unchecked(format!("{}{}", start, local)).into());
                                self
                            } else {
                                self.error(errors, format!("The prefix {}: has not been declared", prefix))
                            }
                        }
                        ...
                        N3Token::Boolean(v) => {
                            self.terms.push(Literal::from(v).into());
                            self
                        }
                        N3Token::Punctuation("{") => {
                ...
                N3State::LiteralPossibleSuffix { value } => {
                    match token {
                        N3Token::LangTag(lang) => {
                            self.terms.push(Literal::new_language_tagged_literal_unchecked(value, lang.into_inner()).into());
                            self
                        },
                        N3Token::Punctuation("^^") => {
                ...
                N3State::LiteralExpectDatatype { value } => {
                    match token {
                        ...
                        N3Token::PrefixedName(prefix, local) => {
                            if let Some(start) = self.prefixes.get(prefix) {
                                self.terms.push(Literal::new_typed_literal(value, NamedNode::new_unchecked(format!("{}{}", start, local))).into());
                                self
                            } else {
                                self.error(errors, format!("The prefix {}: has not been declared", prefix))
                            }
